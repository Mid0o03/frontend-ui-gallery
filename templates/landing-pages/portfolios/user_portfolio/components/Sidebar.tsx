...
import { FolderOpen, FileText, Folder, Mail, User } from 'lucide-react';
          ...
          {sidebar.menu.map((item) => {
            ...
            return (
              <a
                key={item.id}
                href={`#${item.id}`}
                className="flex items-center gap-3 px-3 py-2 text-[#888] hover:text-[#e9e8ce] hover:bg-[#222] rounded-full transition-colors cursor-pointer group"
              >
